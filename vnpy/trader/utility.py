...
class BarGenerator:
    ...
    def update_tick(self, tick: TickData) -> None:
        ...
        if new_minute:
            ...
            # 新增2026/04/20：给每个bid_price和ask_price单独做non-zero计数
            for i in range(1, 6):
                ...
                if bid_price > 0:
                    ...
                    self.bar.extra_metrics[f"bid_price_{i}_count"] = 0

                if ask_price > 0:
                    self.bar.extra_metrics[f"ask_price_{i}_count"] = 1
                else:
                    self.bar.extra_metrics[f"ask_price_{i}_count"] = 0
            # 新增2026/04/20：给每个bid_price和ask_price单独做non-zero计数
            ...
            metrics['price_time_sum'] += self.last_tick.last_price * delta
            metrics['time_weight_sum'] += delta
            ### 新增2026/03/30
    ...
    def _finalize_custom_metrics(self, bar: BarData) -> None:
        ...
        计算结果：
          - avg_bid_price_1 = bid_price_sum / bid_price_count
          - avg_ask_price_1 = ask_price_sum / ask_price_count
