...
class BarData(BaseData):
    ...
    # 新增2026/04/28：盘口价格：只存 sum + count，不存 avg；
    # TWAP充分统计量：twap = twap_num / twap_den，不直接存 twap。
    sum_bid_price_1: float = 0.0      # 买一价之和
    sum_ask_price_1: float = 0.0      # 卖一价之和
    sum_bid_volume_1: float = 0.0                # 有效买一量之和
    sum_ask_volume_1: float = 0.0                # 有效卖一量之和
    cnt_bid_price_1: int = 0                     # 有效买一价ticks数量
    cnt_ask_price_1: int = 0                     # 有效卖一价ticks数量
    
    sum_bid_price_2: float = 0.0      # 买二价之和
    sum_ask_price_2: float = 0.0      # 卖二价之和
    sum_bid_volume_2: float = 0.0                # 有效买二量之和
    sum_ask_volume_2: float = 0.0                # 有效卖二量之和
    cnt_bid_price_2: int = 0                     # 有效买二价ticks数量
    cnt_ask_price_2: int = 0                     # 有效卖二价ticks数量

    sum_bid_price_3: float = 0.0      # 买三价之和
    sum_ask_price_3: float = 0.0      # 卖三价之和
    sum_bid_volume_3: float = 0.0                # 有效买三量之和
    sum_ask_volume_3: float = 0.0                # 有效卖三量之和
    cnt_bid_price_3: int = 0                     # 有效买三价ticks数量
    cnt_ask_price_3: int = 0                     # 有效卖三价ticks数量

    sum_bid_price_4: float = 0.0      # 买四价之和
    sum_ask_price_4: float = 0.0      # 卖四价之和
    sum_bid_volume_4: float = 0.0                # 有效买四量之和
    sum_ask_volume_4: float = 0.0                # 有效卖四量之和
    cnt_bid_price_4: int = 0                     # 有效买四价ticks数量
    cnt_ask_price_4: int = 0                     # 有效卖四价ticks数量

    sum_bid_price_5: float = 0.0      # 买五价之和
    sum_ask_price_5: float = 0.0      # 卖五价之和
    sum_bid_volume_5: float = 0.0                # 有效买五量之和
    sum_ask_volume_5: float = 0.0                # 有效卖五量之和
    cnt_bid_price_5: int = 0                     # 有效买五价ticks数量
    cnt_ask_price_5: int = 0                     # 有效卖五价ticks数量

    tick_count: int = 0                          # Tick 数量

    twap_num: float = 0.0                        # price_time_sum = Σ(price * duration)
    twap_den: float = 0.0                        # time_weight_sum = Σ(duration)
